...
`.trim();
export function buildMarketSystemPrompt(briefingType = "INTRADAY") {
    if (briefingType === "MORNING") {
        return `${MARKET_SYSTEM_PROMPT}\n\n${MORNING_BRIEFING_ADDENDUM}`;
    }
    return MARKET_SYSTEM_PROMPT;
}
export function buildMarketUserMessage(index = {}, gainers = [], losers = [], sectors = [], globalCues = {}, news = [], userPortfolio = [], briefingType = "INTRADAY") {
    const now = new Date();
...
export function buildMarketMessages(index = {}, gainers = [], losers = [], sectors = [], globalCues = {}, news = [], userPortfolio = [], briefingType = "INTRADAY") {
    return [
        { role: "system", content: buildMarketSystemPrompt(briefingType) },
        buildMarketUserMessage(index, gainers, losers, sectors, globalCues, news, userPortfolio, briefingType),
    ];
}
export function deriveMarketMetrics(gainers = [], losers = [], sectors = [], index = {}) {
    ...
    const breadthRatio = totalMovers > 0 ? parseFloat((gainers.length / totalMovers).toFixed(2)) : 0.5;
    let breadthSignal = "MIXED";
    if (breadthRatio > 0.65)
        breadthSignal = "BROAD_BULLISH";
    else if (breadthRatio < 0.35)
    ...
    const topGainer = gainers.sort((a, b) => (b["change%"] ?? 0) - (a["change%"] ?? 0))[0] ?? null;
    const topLoser = losers.sort((a, b) => (a["change%"] ?? 0) - (b["change%"] ?? 0))[0] ?? null;
    return {
        direction,
        change,
        breadthRatio,
        ...
        dayRange: index.dayHigh && index.dayLow ? `${index.dayLow} – ${index.dayHigh}` : null,
    };
}
