...
`.trim();
export function buildAlertUserMessage(eventType, stock, trigger, news = null, userContext = {}) {
    const payload = {
        event: {
            type: eventType,
        },
...
export function buildAlertMessages(eventType, stock, trigger, news = null, userContext = {}) {
    return [
        { role: "system", content: ALERT_SYSTEM_PROMPT },
        buildAlertUserMessage(eventType, stock, trigger, news, userContext),
    ];
}
export function deriveAlertMeta(eventType, stock, trigger) {
    ...
    const cta = eventType === "EARNINGS" || eventType === "BREAKOUT"
        ? "View Analysis"
        : eventType === "NEWS"
            ? "Read News"
            : "Check Portfolio";
...
export function urgencyToClass(urgency) {
    return urgency === "HIGH"
        ? "urgency-high"
        : urgency === "MEDIUM"
            ? "urgency-medium"
...
export function sentimentToClass(sentiment) {
