...
`.trim();
export function buildNewsUserMessage(article, portfolio = []) {
    const tickers = portfolio.map((p) => typeof p === "string" ? p.toUpperCase() : (p.stock || p.ticker || "").toUpperCase());
    return {
        role: "user",
...
export function buildNewsMessages(article, portfolio = []) {
    return [
        { role: "system", content: NEWS_SYSTEM_PROMPT },
        buildNewsUserMessage(article, portfolio),
    ];
}
export function parseAndValidateNewsOutput(rawOutput) {
