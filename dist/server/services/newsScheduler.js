 ...
 * ╚══════════════════════════════════════════════════════════════════════╝
 */
import { analyzeBatch } from "./newsAnalyzer.js";
const cache = {
    results: [],
    portfolio: [],
    lastRunAt: null,
    ...
    return [
        {
            ...
            description: "Reliance Industries Limited posted a net profit of ₹21,243 crore for Q4 FY26, surpassing analyst estimates of ₹19,500 crore. Strong performance driven by Jio and retail segments.",
            source: "Economic Times",
            timestamp: new Date().toISOString(),
        },
        {
            headline: "RBI keeps repo rate unchanged at 6.5%, maintains accommodative stance",
            description: "The Reserve Bank of India held its benchmark repo rate at 6.5% for the sixth consecutive meeting. Governor cited easing inflation and steady GDP growth as key factors.",
            source: "Mint",
            timestamp: new Date().toISOString(),
        },
        {
            headline: "Infosys revises FY27 revenue guidance downward amid macro uncertainty",
            description: "Infosys lowered its annual revenue growth guidance to 4-6% from 8-10% citing client spending caution in North America and Europe. Management expects recovery in H2 FY27.",
...
export async function runAnalysisCycle() {
    ...
    try {
        ...
        console.log(`[NewsScheduler] ✅ Cycle #${cache.runCount} complete — ${successful.length} analyzed, ${failed.length} failed (${elapsed}s)`);
        const portfolioHits = successful.flatMap((r) => r.companies.filter((c) => c.inUserPortfolio));
        if (portfolioHits.length > 0) {
            console.log(`[NewsScheduler] 💼 Portfolio alerts: ${portfolioHits
                .map((h) => h.ticker)
                .join(", ")}`);
        }
    }
...
export function startScheduler(options = {}) {
    ...
    if (runImmediately) {
        setTimeout(() => {
            runAnalysisCycle().catch((e) => console.error("[NewsScheduler] Initial run error:", e.message));
        }, 30_000);
    }
    cache.nextRunAt = new Date(Date.now() + intervalMs).toISOString();
    schedulerHandle = setInterval(() => {
        runAnalysisCycle().catch((e) => console.error("[NewsScheduler] Interval run error:", e.message));
    }, intervalMs);
}
...
export function getPortfolioAlerts() {
    return cache.results.flatMap((result) => result.companies
