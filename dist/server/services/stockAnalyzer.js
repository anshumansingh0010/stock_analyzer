...
import { buildStockMessages, deriveTechnicalSummary, } from "../prompts/stockPrompt.js";
...
export async function analyzeStock(stock, technical = {}, news = [], userHolding = null, options = {}) {
    ...
    const messages = buildStockMessages(stock, technical, news, userHolding);
...
export async function analyzeStockStream(stock, technical = {}, news = [], userHolding = null, onChunk, options = {}) {
    const client = getLLMClient();
    const provider = process.env.LLM_PROVIDER || "openai";
    const model = options.model || (provider === "gemini" ? "gemini-2.0-flash" : "gpt-4o-mini");
    const derived = deriveTechnicalSummary(stock, technical);
...
export function screenStocks(stocks) {
    return stocks
        .map(({ stock, technical }) => {
        ...
        if (d.trend === "UPTREND")
            score += 2;
        if (d.trend === "DOWNTREND")
            score -= 2;
        if (d.rsiZone === "OVERSOLD")
            score += 1;
        if (d.rsiZone === "OVERBOUGHT")
            score -= 1;
        if (d.macdDirection === "BULLISH")
            score += 1;
        if (d.macdDirection === "BEARISH")
            score -= 1;
        if (d.volumeConviction === "HIGH")
            score += 1;
        if (d.volumeConviction === "LOW")
            score -= 1;
        return {
            ticker: stock.ticker,
            name: stock.name,
