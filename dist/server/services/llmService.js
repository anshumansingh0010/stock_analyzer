...
import { buildSystemMessages, validateContext } from "../prompts/masterPrompt.js";
let llmClient = null;
...
export async function withRetry(fn, maxRetries = 5) {
    ...
    for (let attempt = 0; attempt <= maxRetries; attempt++) {
        try {
            ...
            console.warn(`[LLM] Rate limit hit — retrying in ${(delay / 1000).toFixed(1)}s (attempt ${attempt + 1}/${maxRetries})`);
            await new Promise((r) => setTimeout(r, delay));
        }
    }
    throw lastError;
}
export async function chat(userQuery, context = {}, chatHistory = [], options = {}) {
...
export async function chatStream(userQuery, context = {}, chatHistory = [], onChunk, options = {}) {
    const client = getLLMClient();
    const { warnings } = validateContext(context);
    const systemMessages = buildSystemMessages(context);
    const messages = [
