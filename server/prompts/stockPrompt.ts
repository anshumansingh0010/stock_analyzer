...
export interface StockMessageRole {
...
export interface TechnicalSummary {
  ...
  volumeConviction: "HIGH" | "LOW" | "NORMAL";
...
export function buildStockUserMessage(
...
export function buildStockMessages(
  stock: BasicStockData,
  technical: TechnicalData = {},
  news: StockNewsItem[] = [],
  userHolding: UserHoldingData | null = null
...
export function deriveTechnicalSummary(
  stock: BasicStockData,
  technical: TechnicalData
): TechnicalSummary {
  ...
  let volumeConviction: "HIGH" | "LOW" | "NORMAL" = "NORMAL";
  let volumeRatio: number | null = null;
  if (volume && avgVolume && avgVolume > 0) {
    volumeRatio = parseFloat((volume / avgVolume).toFixed(2));
    if (volumeRatio > 1.5) volumeConviction = "HIGH";
    else if (volumeRatio < 0.7) volumeConviction = "LOW";
  }

  let macdDirection: "BULLISH" | "BEARISH" | "NEUTRAL" = "NEUTRAL";
  if (MACD != null && signal != null) {
    if (MACD > signal) macdDirection = "BULLISH";
    else if (MACD < signal) macdDirection = "BEARISH";
  }
