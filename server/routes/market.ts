...
import {
  ...
  analyzeMarketStream,
  getBriefingType,
  generateQuickTakeaway,
} from "../services/marketAnalyzer.js";
import { deriveMarketMetrics, BriefingType } from "../prompts/marketPrompt.js";
...
function extractPayload(body: any) {
  return {
    ...
    briefingType: (body?.briefingType as BriefingType) ?? (getBriefingType() as BriefingType),
  };
}
...
export default async function marketRoutes(fastify: FastifyInstance): Promise<void> {
  fastify.post("/explain", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const meta = await analyzeMarketStream(
        index,
        gainers,
        losers,
        sectors,
  ...
  fastify.post("/explain/sync", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      const {
        index,
        gainers,
  ...
  fastify.get("/briefing", async (_request: FastifyRequest, reply: FastifyReply) => {
  ...
  fastify.post("/briefing", async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      const VALID_TYPES: BriefingType[] = ["MORNING", "INTRADAY", "CLOSING"];
      const finalType: BriefingType = VALID_TYPES.includes(requestedType)
        ? requestedType
        : (getBriefingType() as BriefingType);
  ...
  fastify.get("/quicktake", async (request: FastifyRequest, reply: FastifyReply) => {
  ...
  fastify.get("/briefing-type", async (_request: FastifyRequest, reply: FastifyReply) => {
    return reply.send({
      success: true,
      briefingType: getBriefingType(),
